...
bool GLSLProgram::isUsing()
{
    GLint currentProgram = 0;
    glGetIntegerv(GL_CURRENT_PROGRAM, &currentProgram);
    return static_cast<GLuint>(currentProgram) == handle;
}
...
void GLSLProgram::setSampler2D(const char* name, unsigned int texture) 
{
    glActiveTexture(GL_TEXTURE0 + texUnitsIndex);
    glBindTexture(GL_TEXTURE_2D, texture);
    this->setUniform(name, texUnitsIndex);
    texUnitsIndex++;
}
void GLSLProgram::setSampler3D(const char* name, unsigned int texture) 
{
    glActiveTexture(GL_TEXTURE0 + texUnitsIndex);
